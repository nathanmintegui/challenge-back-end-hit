...
class Planeta(val id: Int, val nome: String, val clima: String, val terreno: String)
...
class PlanetaController(val db: JdbcTemplate) {
        ...
        fun inserir(@RequestBody request: CriarPlanetaRequest): ResponseEntity<Response> {
                ...
                if (request.clima.isNullOrEmpty()) {
                        return ResponseEntity(
                                        Response(
                                                        message = "campo clima é obrigatorio",
                                                        body = null,
                                                        status = 400
                                        ),
                                        HttpStatus.BAD_REQUEST
                ...
                if (request.terreno.isNullOrEmpty()) {
                        return ResponseEntity(
                                        Response(
                                                        message = "campo terreno e obrigatorio",
                                                        body = null,
                                                        status = 400
                                        ),
                                        HttpStatus.BAD_REQUEST
        ...
        fun deletar(@PathVariable id: Int): ResponseEntity<Response> {
                ...
                val isPlanetaExistente =
                                db.queryForObject<Int>(
                                                "SELECT COUNT(*) FROM PLANETAS WHERE id = ?",
                                                arrayOf(id),
                                                Int::class.java
                                )
